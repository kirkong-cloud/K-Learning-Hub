...
function makeToken(user) {
  const payload = Buffer.from(JSON.stringify({ id: user.id, email: user.email, role: user.role, exp: Date.now() + TOKEN_TTL_HOURS * 60 * 60 * 1000 })).toString("base64url");
...
function readDatabase() {
  if (!fs.existsSync(DATA_FILE)) {
    ...
    fs.writeFileSync(DATA_FILE, JSON.stringify({ topics: [], quizBank: [], scores: [], users: [], pendingOtps: [], paymentRequests: [], creditTransactions: [], auditLogs: [], professorProfile: {}, paymentSettings: { pricePerQuestion: 2, qrImagePath: "assets/gcash-qr.jpg" }, siteSettings: { announcement: "Welcome to the Learning Hub. Check your quiz credits and continue your lessons regularly." } }, null, 2));
  }
  ...
  if (process.env.ADMIN_EMAIL && process.env.ADMIN_PASSWORD) {
    seedUsers.push({ email: process.env.ADMIN_EMAIL.toLowerCase(), password: process.env.ADMIN_PASSWORD, role: "admin", name: process.env.ADMIN_NAME || "Administrator", contactNumber: "" });
  } else if (!IS_PRODUCTION && process.env.ENABLE_DEMO_ACCOUNTS !== "false") {
    seedUsers.push({ email: "admin@example.com", password: "Admin123!", role: "admin", name: "Admin", contactNumber: "" });
  }
  if (!IS_PRODUCTION && process.env.ENABLE_DEMO_ACCOUNTS === "true") {
    seedUsers.push({ email: "student@example.com", password: "Student123!", role: "student", name: "Student Demo", contactNumber: "09123456789" });
  }
  for (const seed of seedUsers) {
    if (!db.users.some(u => String(u.email).toLowerCase() === seed.email)) {
      db.users.push({
        id: uuidv4(),
        name: seed.name || "",
...
app.get("/", (req, res) => {
  res.json({ status: "ok", message: "K Learning Hub backend is running.", frontend: process.env.FRONTEND_URL || "Set FRONTEND_URL in Render" });
});
...
app.post("/api/auth/login", rateLimit({ windowMs: 15 * 60 * 1000, max: 8 }), (req, res) => {
  const email = String(req.body.email || "").trim().toLowerCase();
  const password = String(req.body.password || "");
  const db = readDatabase();
  ...
  if (user.role === "student" && user.approvalStatus !== "approved") {
    logAudit(db, req, "login_blocked_pending_approval", { email });
    writeDatabase(db);
  ...
  logAudit(db, req, "login_success", { userId: user.id, email: user.email, role: user.role });
  writeDatabase(db);
...
app.post("/api/topics", requireAuth, requireAdmin, (req, res) => {
  ...
  logAudit(db, req, "topic_created", { topicId: topic.id, title: topic.title });
  writeDatabase(db);
...
app.post("/api/payments/request", requireAuth, rateLimit({ windowMs: 15 * 60 * 1000, max: 10 }), (req, res) => {
  ...
  if (reference && !/^[A-Za-z0-9\-_.]{6,40}$/.test(reference)) return res.status(400).json({ error: "Please enter a valid GCash reference number." });
  if (reference && (db.paymentRequests || []).some(r => String(r.reference || "").toLowerCase() === reference.toLowerCase() && r.status !== "rejected")) {
    return res.status(409).json({ error: "This GCash reference number was already submitted. Please verify the reference or contact the admin." });
  }
  ...
  if (receiptData && Buffer.byteLength(receiptData, "utf8") > MAX_RECEIPT_BYTES) return res.status(400).json({ error: "Receipt screenshot is too large. Please upload a compressed image." });
  const request = {
    id: uuidv4(),
    userId: user.id,
    name: user.name || "",
  ...
  logAudit(db, req, "payment_request_created", { paymentRequestId: request.id, itemCount, amount: request.amount, reference: reference ? "provided" : "none" });
  writeDatabase(db);
...
app.post("/api/admin/payment-requests/:id/approve", requireAuth, requireAdmin, (req, res) => {
  ...
  logAudit(db, req, "payment_approved_credits_synced", { paymentRequestId: request.id, creditsAdded: sync.addedNow, studentEmail: sync.user.email });
  writeDatabase(db);
  res.json({
    request,
...
app.post("/api/admin/payment-requests/:id/reject", requireAuth, requireAdmin, (req, res) => {
  ...
  logAudit(db, req, "payment_rejected", { paymentRequestId: request.id });
  writeDatabase(db);
  res.json(request);
});
...
app.get("/api/admin/audit-logs", requireAuth, requireAdmin, (req, res) => {
  ...
  res.json((db.auditLogs || []).slice(-limit).reverse());
});

app.use((err, req, res, next) => {
  console.error(err);
  res.status(500).json({ error: IS_PRODUCTION ? "Server error." : err.message });
});
